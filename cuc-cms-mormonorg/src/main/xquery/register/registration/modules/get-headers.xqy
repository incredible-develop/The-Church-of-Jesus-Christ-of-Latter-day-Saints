...
return (
    for $site in $search
    let $site-name as xs:string := fn:normalize-space(xs:string($site/basic-data/site-name))
    let $server as xs:string := fn:normalize-space(xs:string($site/basic-data/server))
    let $id as xs:string := fn:normalize-space(xs:string($site/@id))
    let $port as xs:string := fn:normalize-space(xs:string($site/basic-data/port))
    let $site-shared-prefix as xs:string := fn:normalize-space(xs:string($site/basic-data/shared-prefix))
    let $translation as xs:string? := fn:normalize-space(xs:string($site/translation-checkbox))
    let $services as xs:string? := fn:normalize-space(xs:string($site/services-checkbox))
