...
module namespace json-snippet = "http://lds.org/code/shared/lds-edit/new-content-manager/json-snippet";

import module namespace functx = "http://www.functx.com" at "/MarkLogic/functx/functx-1.0-nodoc-2007-01.xqy";

import module namespace content-settings = "http://lds.org/code/shared/lds-edit/new-content-manager/content-settings" at "content-settings.xqy";
import module namespace content-search = "http://lds.org/code/shared/lds-edit/new-content-manager/content-search" at "content-search.xqy";
...
import module namespace content-functions = "http://lds.org/code/shared/lds-edit/new-content-manager/content-functions" at "content-functions.xqy";
...
declare function getPublishRights( $file as element() ) as xs:string {
    json:keyEscapedValue("canPublish", ldsemeta:correlation-can-publish($file))
};
...
declare function getPreviewUrl(
    ...
    default return (
        json:keyEscapedValue("previewUrl", (core:build-url(ldsemeta:get-document-uri($file), (ldsemeta:get-document-locale($file), "eng")[1], ()), "")[1] )
    )
};
...
declare function getRoot(
    ...
    if (fn:node-name($file) != xs:QName('resources')) then (
...
declare function getLanguage(
    ...
    json:keyValue("language", util:get-full-language-name-by-locale(ldsemeta:get-document-locale($file)) ) 
};
...
declare function getPublished(
    ...
    json:keyValue("datePublished", content-search:formatDate( ldsemeta:get-publish-date($file)/@date) )
};
...
declare function get-date-created($file as node()) as xs:string {
    json:keyValue("dateCreated", content-search:formatDate($file/ldse:ldse-meta/ldse:created/@date))
};
...
declare function get-date-modified($file as node()) as xs:string {
    json:keyValue("dateModified", content-search:formatDate($file/ldse:ldse-meta/ldse:last-modified/@date))
};

declare function get-unpublished-date($file as node()) as xs:string {
    json:keyValue("dateUnpublished", content-search:formatDate($file/ldse:ldse-meta/ldse:unpublish-date/@date))
};
...
declare function get-comment-date($file as node()) as xs:string {
    json:keyValue("dateCommented", content-search:formatDate($file/@date))
};
...
declare function get-comment-type($file as node()) as xs:string {
    json:keyValue("fileType", $file/hldse:current/hldse:ldse-meta/hldse:document/@type)
};

declare function get-comment-title($file as node()) as xs:string {
    json:keyValue("title", $file/hldse:current/hldse:ldse-meta/hldse:document/@title)
};
