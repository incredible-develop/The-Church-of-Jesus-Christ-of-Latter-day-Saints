...
module namespace export-functions = "http://lds.org/code/shared/lds-edit/new-content-manager/export"; 
...
declare function buildExportRow($file as element(), $status as xs:string?) as element(row) {
    ...
    let $publisher as xs:string? := fn:string(ldsemeta:get-publish-date($file)/@username)
    let $publish-date as xs:string? := fn:format-dateTime(ldsemeta:get-publish-date($file)/@date, "[Y0001]-[M01]-[D01]", 'eng', (), ())
    let $creator as xs:string := fn:string(ldsemeta:get-created-date($file)/@username)
    let $created-date as xs:string? := fn:format-dateTime(ldsemeta:get-created-date($file)/@date, "[Y0001]-[M01]-[D01]", 'eng', (), ())
    let $unpublisher as xs:string? := fn:string(ldsemeta:get-unpublish-date($file)/@username)
