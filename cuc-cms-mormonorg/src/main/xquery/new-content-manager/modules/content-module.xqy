...
module namespace content-module ="http://lds.org/code/shared/lds-edit/new-content-manager/content-module";
              ...
              { if ( fn:empty($column/@link) and fn:empty($column/@read-more)) then (
                  fn:concat("{{",$data,"}}") 
                    ...
                    return (
                      ...
                      return (
                          fn:concat('{{#if this.',$path,'}}'),
                               fn:concat('{{#if this.option}}'),
                                    <a href="{$settings:shared-prefix}/form{{{{previewLocale}}}}&amp;id={{{{id}}}}" onclick="ICE.postLink(this); return false;" data-post.status="{{{{dataPostStatus}}}}" data-post.uri="{{{{uri}}}}" data-post.page="{{{{uri}}}}" data-post.option="{{{{option}}}}">
                                         {{{{{$data}}}}}
                                    </a>,
                               '{{else}}',
