...
if (ac:has-permission('ldse:view-reports-admin', '', '')) then (
    let $title as xs:string := "Page Status Report"
    let $thisPage as xs:string := "shared/lds-edit/reports/page-status-report"
    ...
    return
        core:template-apply(
            ...
            <page>
                ...
                <button-groups>
                    <button-group>
                        <button id="pageStatusReportButton" class="ldse-button primary ldse-icon-download ldse-modal-trigger" onclick="SM.updatePageStatusExportType();  modal.trigger(&quot;close&quot;); SM.pageStatusReportExport(); return false;">Save Report</button>
                    </button-group>
                </button-groups>
                <options></options>
            </page>
