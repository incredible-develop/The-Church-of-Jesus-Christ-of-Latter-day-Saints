...
module namespace content-workflow = "http://lds.org/code/shared/lds-edit/content/content-workflow";
...
import module namespace snip = "http://lds.org/code/shared/lds-edit/content/json-snippet" at "json-snippet.xqy";
...
declare function get-step-query($step as element(ldse:step)) as cts:query* {
            ...
            cts:and-query((
                 ...
                 if ($query/@status eq "published") then (
                    ...
                    cts:and-query ((
                         ...
                         cts:or-query(
                         	(cts:element-attribute-value-query(xs:QName("ldse:submission"), xs:QName("edited"), "true", 'exact'),
                         	cts:not-query(
	                         	cts:element-query(xs:QName("ldse:submission"), cts:and-query(())
                         		)
                         	))
                         )
                 ...
                 ) else if($query/@status eq "submission-created") then (
                    cts:and-query((
						cts:element-attribute-value-query(xs:QName("ldse:submission"), xs:QName("status"), "created", "exact")
					))
				) else if ( $query/@status eq "submission-submitted" ) then (
    				cts:and-query((
    					cts:element-attribute-value-query(xs:QName("ldse:submission"), xs:QName("status"), "submitted", "exact"),
    					cts:not-query(
    						cts:element-attribute-value-query( xs:QName("ldse:submission"), xs:QName("edited"), "true", "exact")
    					)
    				))
    ...
    let $cts-query as cts:query* :=
    	cts:and-query((
			...
			if ( fn:not($query/@status eq ("submission-created", "submission-submitted")) ) then (
				cts:not-query(
					cts:element-attribute-value-query(
...
declare function build-function($element as element()) as xdmp:function {
    let $ns as xs:string := ($element/@ns[. ne ''], "http://lds.org/code/shared/lds-edit/content/content-workflow")[1]
...
declare function remove-file(
    ...
    if ( fn:exists($action-element) ) then (
        ...
        let $save as element() := core:preform-action-and-update($ldse-action, $file)
        return (),
        '{"success":true}'
    ) else (
...
declare function publish-files(
    ...
    let $publish as element()* := core:preform-action-and-update($ldse-action, $files)
    return (
        '{"success":true}'
    )
};
