...
module namespace content-search = "http://lds.org/code/shared/lds-edit/content/content-search";
...
import module namespace snip = "http://lds.org/code/shared/lds-edit/content/json-snippet" at "/content/modules/json-snippet.xqy";
...
import module namespace content-settings = "http://lds.org/code/shared/lds-edit/content/content-settings" at "/content/modules/content-settings.xqy";
...
declare variable $curr-site as xs:string? := xdmp:get-request-field('site');
...
declare function content-search:getOptions($filterQuery as cts:query*, $useFacets as xs:boolean, $pageNum as xs:string) as element(search:options) {
    let $selectedStatus as xs:string? := xdmp:get-request-field("status",'draft')
    ...
    <options xmlns="http://marklogic.com/appservices/search">
        ...
        {
            if ( fn:exists($transResults) ) then (
                $transResults
            ) else (
                <transform-results apply="all-default-json" ns="http://lds.org/code/shared/lds-edit/content/json-snippet"
                       at="/content/modules/json-snippet.xqy">
                    <param>
                        <pageNum>{$pageNum}</pageNum>
                    </param>
                </transform-results>
...
declare function content-search:uriFilter(
    $uris as xs:string*,
    $status as xs:string?
) as cts:query? {
        ...
        cts:or-query((
           ...
           return (
               if ( $URI = 'rice') then (
                   ...
                   cts:element-attribute-range-query($root, xs:QName("uri"),  "=", "/", "collation=http://marklogic.com/collation/")
               ) else (
                   cts:and-query((
                       cts:element-attribute-range-query($root, xs:QName("uri"), ">=", $uri, "collation=http://marklogic.com/collation/"),
                       cts:element-attribute-range-query($root, xs:QName("uri"), "<=", fn:concat($uri, "~"), "collation=http://marklogic.com/collation/")
                   ))
                )
           )
       ))
...
declare function content-search:categoryFilter($categories as xs:string*, $status as xs:string?) as cts:query?{
    if ( fn:exists($categories) ) then (
        cts:element-attribute-value-query(
            xs:QName("ldse:document"),
            xs:QName("categories"),
            for $category as xs:string in $categories
            return(fn:concat("*",$category,"*")),
            ("wildcarded")
        )
    ) else ()
};
...
declare function content-search:siteFilter(
    $site as xs:string?
) as cts:query? {
...
declare function content-search:jsonSearch(
    $renderSummary as xs:boolean
) as xs:string? {
    content-search:search(fn:false(), $renderSummary)[1]
};
...
declare function content-search:getFilterQuery($status as xs:string?, $pdQuery as xs:string?) as cts:query{
    ...
    let $statusFilter as cts:query* :=
        if ( fn:empty($selectedStatus) ) then (
            content-workflow:get-all-step-queries()
...
(:                    content-search:categoryFilter(xdmp:get-request-field("categories[]")[. ne ''], $selectedStatus),:)
                    content-search:siteFilter(xdmp:get-request-field("site")[. ne '']),
...
declare private function content-search:search($generateFacets as xs:boolean, $renderSummary as xs:boolean) as item()? {
    ...
    let $searchResponse as element(search:response)* :=
            try {
                search:search($searchQuery, content-search:getOptions($filterQuery, $generateFacets, xs:string($pageIndex)), ($pageIndex - 1) * $pageSize + 1, $pageSize)
            } catch ($e) {}
        ...
        let $r as item()* :=
            if ($generateFacets) then (
                ...
                json:arr(($summary,
                    ...
                    xs:string(fn:ceiling($totalPagesFound div $pageSize)),
    				xs:string($pageIndex),
					xs:string($totalPagesFound)
    			))
            )
        return $r
    )
...
declare function content-search:buildSiteSelect() as element(select) {
    <select>{
...
declare function content-search:generateQueryStep(
    ...
    let $json as xs:string* :=
	    if ( fn:empty($selectedStatus) or $selectedStatus = $step/@name ) then (
	        json:obj((
	            json:keyValue("title", xs:string($step/@title)),
	            json:keyValue("name", $step-title),
	            if ( fn:not($selectedStatus = "comments") ) then (
