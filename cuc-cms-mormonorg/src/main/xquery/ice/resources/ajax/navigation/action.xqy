...
(: item object :)
    ...
    declare variable $ids as xs:string+ := xdmp:get-request-field("ids[]", "ldsorg-navigation");


declare variable $has-permission as xs:boolean :=  ac:is-action-allowed($action, $locale, $uri);
declare variable $files as element()* := cts:search(/channels[@name eq $ids and @locale eq $locale], core:get-filter-query());
...
if ($has-permission) then (
        ...
        let $xml as element(response) := 
            <response>
                ...
                <count>{fn:count($files[$action = "ldse:publish"])}</count>
                <name>{$name}</name>
