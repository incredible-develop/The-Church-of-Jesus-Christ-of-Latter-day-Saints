...
declare variable $files as element(resources)* := 
    if ($action = "ldse:publish") then (
        cts:search(/resources,
            cts:and-query((
                ...
                cts:element-attribute-value-query(xs:QName('entry'), xs:QName('status'), "edited", 'exact')
            ))
        )
    ) else ();
...
if ($has-permission) then (
        let $response as item()* :=
            if ( $action = "ldse:publish" and fn:exists($files)) then (
                ...
                return 
                    if ($lock) then (
                        try {
                            let $perform as item()* :=
                                for $file as element(resources) in $files
