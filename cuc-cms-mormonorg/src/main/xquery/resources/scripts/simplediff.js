...
var htmlDiff = function(before, after, noDels, noIns) {
	var noDel = noDels || false,
		noIn = noIns || false;
    /*
...
function domDiff(before, after) {
	var bPath = getXPath(before[0]),
		aPath = getXPath(after[0]),
		bPaths = {},
		aPaths = {},
		diffs = { before: [], after: [] };
	before.find('*').each(function(){
		var e = $(this).eq(0),
			path = getXPath(this).replace(bPath, '');
		bPaths[path] = e;
	});
	after.find('*').each(function(){
		var e = $(this),
			path = getXPath(this).replace(aPath, '');
		aPaths[path] = e;
	});
	
	for (var path in bPaths) {
	   if (bPaths.hasOwnProperty(path)) {
	      var obj = bPaths[path],
	          compare = aPaths[path];
	      ...
	      if (compare != undefined) {
    	     ...
    	     obj.contents().filter(function() { 
	    	 	if (this.nodeType === 3 ){
	    	 		bContents += this.nodeValue;
	    	 		return true;
	    	 	}
    	     	return false;
    	     });
    	     compare.contents().filter(function() { 
 	    	 	if (this.nodeType === 3 ){
 	    	 		aContents += this.nodeValue;
 	    	 		return true;
 	    	 	}
     	     	return false;
     	     });
	    	  if ( bContents != aContents) {
	    		  diffs.before.push(obj);
	    		  diffs.after.push(compare); 
	    	  }
	      } else {
	    	  diffs.before.push(obj);
	      }
	...
	for (var path in aPaths) {
	   if (aPaths.hasOwnProperty(path)) {
	      var obj = aPaths[path],
	          compare = bPaths[path];
	      ...
	      if (compare != undefined) {
		     ...
		     compare.contents().filter(function() { 
		    	 	if (this.nodeType === 3 ){
		    	 		bContents += this.value;
		    	 		return true;
		    	 	}
	 	     	return false;
	 	     });
	    	  if ( bContents != aContents) {
	    		  diffs.after.push(obj);
	    		  diffs.before.push(compare);
	    	  }
	      } else {
	    	  diffs.after.push(obj);
	      }
